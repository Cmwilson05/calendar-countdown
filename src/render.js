    ...
    return `
        <div class="flex items-start gap-6 relative group cursor-pointer" onclick="handleEventClick('${event.id || event.tempId}')">
            ...
            <div class="flex-1 pt-1">
                <div class="font-bold text-2xl text-black dark:text-white leading-tight mb-0.5">${event.title}</div>
                <div class="flex items-baseline gap-2">
                    ${state.showDays ? `<div class="text-gray-400 font-semibold text-xl">${days === 0 ? 'Today' : (days > 0 ? `In ${days} days` : `${Math.abs(days)} days ago`)}</div>` : ''}
                    ${state.showNotes && event.notes ? `<div class="text-sm text-gray-500 italic">${event.notes}</div>` : ''}
                </div>
            </div>
        </div>
    `;
